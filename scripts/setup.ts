...
// Update this function to use .env instead of .dev.vars
async function updateEnvWithSecret() {
    ...
    try {
        ...
        if (!envContent.includes("AUTH_SECRET")) {
            ...
            console.log("\x1b[33mSecret appended to .env file.\x1b[0m");
        } else {
            console.log("\x1b[31mAUTH_SECRET already exists in .env\x1b[0m");
        }
    } catch (error) {
        console.error("\x1b[31mError updating .env file:", error, "\x1b[0m");
        cancel("Operation cancelled.");
    ...
    outro(".env updated with secure AUTH_SECRET.");
}
...
async function main() {
    try {
        ...
        await updateEnvWithSecret();
        await runDatabaseMigrations(dbName);

        console.log("\x1b[33mRunning npm run dev command...\x1b[0m");
        spawnSync("npm", ["run", "dev"], { stdio: "inherit" });
    } catch (error) {
        console.error("\x1b[31mError:", error, "\x1b[0m");
        cancel("Operation cancelled.");
    }
